      ...
      body: SafeArea(
        child: Row(
          children: [
            ...
            Expanded(child: _WorkspaceContent(database: database, institution: institution)),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(children: [Container(width: 36, height: 36, decoration: BoxDecoration(color: AppColors.primary, borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.verified_outlined, color: Colors.white, size: 21)), const SizedBox(width: AppSpacing.sm), Text(AppEnvironment.appName, style: Theme.of(context).textTheme.titleMedium)]),
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Semantics(
        ...
        child: Container(
          height: 44,
          decoration: BoxDecoration(color: selected ? AppColors.primaryLight : Colors.transparent, borderRadius: BorderRadius.circular(AppRadius.input)),
          child: ListTile(
            dense: true,
...
class _WorkspaceContent extends StatelessWidget {
  const _WorkspaceContent({this.database, this.institution});
      ...
      child: SingleChildScrollView(
        ...
        child: ConstrainedBox(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Row(children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(institution?.name ?? 'Good morning', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.textSecondary)), const SizedBox(height: AppSpacing.xs), Text('Create and manage your certificates', style: Theme.of(context).textTheme.headlineLarge)])), if (database?.isOpen ?? false) ...[const AppStatusBadge(label: 'Offline ready'), const SizedBox(width: AppSpacing.md)], IconButton(tooltip: 'Notifications', onPressed: () {}, icon: const Icon(Icons.notifications_none_outlined)), const SizedBox(width: AppSpacing.xs), IconButton(tooltip: 'Settings', onPressed: () {}, icon: const Icon(Icons.settings_outlined))]),
              const SizedBox(height: AppSpacing.xxl),
              Row(children: [Expanded(child: AppPrimaryButton(label: 'New project', icon: Icons.add, onPressed: () {})), const SizedBox(width: AppSpacing.md), Expanded(child: AppSecondaryButton(label: 'Import project', icon: Icons.file_upload_outlined, onPressed: () {}))]),
              const SizedBox(height: AppSpacing.xxl),
              ...
              const SizedBox(height: AppSpacing.md),
              const Row(children: [Expanded(child: _MetricCard(icon: Icons.image_outlined, value: '0', label: 'Templates')), SizedBox(width: AppSpacing.md), Expanded(child: _MetricCard(icon: Icons.text_fields_outlined, value: '0', label: 'Fonts')), SizedBox(width: AppSpacing.md), Expanded(child: _MetricCard(icon: Icons.workspace_premium_outlined, value: '0', label: 'Certificates'))]),
            ],
          ),
        ),
      ),
...
class _ProjectPreviewCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => AppSurfaceCard(padding: const EdgeInsets.all(AppSpacing.md), child: ListTile(contentPadding: EdgeInsets.zero, leading: Container(width: 48, height: 48, decoration: BoxDecoration(color: AppColors.surfaceMuted, borderRadius: BorderRadius.circular(AppRadius.input)), child: const Icon(Icons.description_outlined, color: AppColors.primary)), title: Text(title, style: Theme.of(context).textTheme.titleMedium), subtitle: Padding(padding: const EdgeInsets.only(top: AppSpacing.xxs), child: Text(metadata)), trailing: AppStatusBadge(label: status, color: statusColor, backgroundColor: statusBackground), onTap: () {}));
}
